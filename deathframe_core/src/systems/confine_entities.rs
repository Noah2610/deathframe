...
impl<'a> System<'a> for ConfineEntitiesSystem {
    ...
    fn run(
        ...
        for (_, confined, transform, size_opt, loadable_opt, loaded_opt) in (
            &entities,
            &confined_store,
            &mut transforms,
            sizes.maybe(),
                ...
                if let Some(size) = size_opt {
                    ...
                    if rect.left < confined.rect.left {
                        transform.set_translation_x(
                            confined.rect.left + half_size.w,
                        );
                    }
                    if rect.right > confined.rect.right {
                        transform.set_translation_x(
                            confined.rect.right - half_size.w,
                        );
                    }
                    if rect.bottom < confined.rect.bottom {
                        transform.set_translation_y(
                            confined.rect.bottom + half_size.h,
                        );
                    }
